export const experiences = [
  {
    company: "Alza",
    role: "Software Engineer",
    dates: "July 2023 - Aug 2023",
    description: [
      `Helped launch and scale a neobank to 2500+ customers and 5M+ in transaction volume`,
      `Built cross border and ACH transfer flows including 3rd party integrations, transaction data models, recipient compliance checks, user notifications, and internal dashboards`,
      `Developed critical cron jobs and back-end flows for in-house lifecycle marketing, customer due diligence, algorithmic risk monitoring, and partner data ingestion`,
      `Worked on devops and infrastructure improvements, including setup scripts, streamlined cron job deployment with Kubernetes, and increased service availability`,
    ],
    link: "",
    thanks: `Alza was my bootcamp as a junior software engineer and aspiring founder. The team - Arturo, David, etc. were incredible mentors and people to work with.`,
  },
  {
    company: "Google",
    role: "Associate Product Management Intern",
    dates: "May 2022 - Aug 2022",
    description: [
      `Researched and designed hotel advertisement algorithm that surfaces most relevant booking dates based on search input and availability, including Figma mockups and PRD`,
      `Pushed forward hotel ads experiment through approvals, implementation, and partner strategy development, including auction pricing model and predicted ARR impact`,
    ],
    link: "",
    thanks: `I'm grateful to my mentors, especially Anurag Jain, for teaching me to ask the right product questions, model outcomes, and test hypotheses.`,
  },
  {
    company: "Quansa",
    role: "Software Engineering Intern",
    dates: "Jan-Feb 2022",
    description: [
      `Developed a reconcilation system for Quansa salary advances and company invoices, including a bank 
            scraper bot and StarkBank integration`,
      `Built automatic invoice generation and payment tracking dashboard`,
    ],
    link: "https://www.quansa.io/",
    thanks: `I'm super thankful for the Quansa team for bringing me on for a short but incredibly impactful experience. 
        I realized how much I love building in fintech, especially making a direct positive impact on people. Fran and the rest of
        the engineering team helped me learn how to build reliable systems from scratch and Mafalda and Gonzalo pushed me to grow
        as a communicator and problem solver.`,
  },
  {
    company: "MIT CSAIL: Program Synthesis Lab",
    role: "Undergraduate Researcher",
    dates: "Feb-May 2022",
    description: [
      `Integrated user interactivity into algorithm that generates a class diagram for a desired 
      web/mobile app with GPT-3 and NLP`,
      `Developing webapp for user to collaborate with GPT-3 on generating an app design with text input 
      and selection prompts`,
    ],
    link: "https://arxiv.org/abs/2210.11468",
    thanks: `This project was exciting to me because of the potential for it to transform the no-code/low-code space
    and because of the interesting challenges it has in refining the algorithm and harnessing GPT-3 in the right ways.
    I've also learned a ton working with Alex Gu, who has taught me how to ask the right research questions and build 
    creative programs to test hypotheses.`,
  },
  {
    company: "Figma",
    role: "Software Engineering Intern",
    dates: "June-Aug 2021",
    description: [
      `Redesigned Figma signup and onboarding experiences with design for
    front-end, database, APIs, and state management`,
      `Implemented
    education admin and verification features, including banners,
    dashboard tips, and file permissions logic`,
    ],
    link: "https://twitter.com/figma/status/1430985467030351875",
    thanks: `I grew so much with this project, learning how to build a feature end-to-end, from the user research discussion to the full launch. Special
    thanks to my amazing mentor Diana Chang, who helped me improve tons in my problem-solving and coding, my manager Matt Dailey, who supported me with my
    internship goals and taught me a lot about leading teams effectively and with empathy, and the rest of the Figma Growth team <3`,
  },
  {
    company: "Facebook",
    role: "Software Engineering Intern",
    dates: "Jan-April 2021",
    description: [
      `Developed search experience with filters and data logging for
    employees to find relevant roles at Facebook for referrals`,
      `Implemented full-stack system for ML recommendations for positions
    based on uploaded resume on referral forms`,
    ],
    link: "",
    thanks: `This was my first time launching features to prod during an internship and I learned a lot about code planning, testing, and iterating. 
    I'm super grateful for my mentor, Sara Sahabi, for supporting me 24/7 and helping me navigate a huge codebase, my team 
    lead Carlos Sarmiento, for teaching me a lot about good code design, and my team PM Vik Mehta, for helping me explore my PM interests as well.`,
  },
  {
    company: "Microsoft",
    role: "Software Engineering Intern",
    dates: "May-Aug 2020",
    description: [
      `Redesigned Azure Static Web Apps to be able to integrate into other
    low-code/no-code platforms and code editor extensions`,
      `Developed back-end infrastructure and APIs for app deployment and content
    distribution, cutting deployment time by up to 75%`,
    ],
    link: "https://azure.microsoft.com/en-us/products/app-service/static/",
    thanks: `I was such a noob at the start of this internship, just learning the ropes of version control, debugging, and contributing to a large codebase, and
    my mentor, Joseph Lin, supported me a ton throughout all of that. I'm also grateful for my manager, Mitch Webster, for supporting me as well and
    including me in the design discussions, and for Zhe Yang, for making me feel so welcome on the team.`,
  },
  {
    company: "PatientPort",
    role: "Software Engineering Intern",
    dates: "May-Aug 2020",
    description: [
      `Enabled patient communications and data collection through web application for healthcare centers in Germany`,
      `Worked on end-to-end process for appointment booking and post-surveys in order to facilitate telemedicine at a client’s office`,
    ],
    link: "",
    thanks: `This was a super cool opportunity to learn how to build features for a specific customer and how a startup team runs on a daily basis. 
    Huge thank you to Aparna Pande for bringing me on and helping me improve as a dev in my early days :)`,
  },
  {
    company: "MIT CSAIL: Distributed Robotics Lab",
    role: "Undergraduate Researcher",
    dates: "March-June 2020",
    description: [
      `Developer fabric defect detection device with binary and siamese
    machine learning models, using Python and TensorFlow`,
      `Implemented ML explainability with visualization using a back-propagation algorithm
    to assist workers in textile manufacturing`,
    ],
    link: "https://www.csail.mit.edu/research/distributed-robotics-laboratory",
    thanks: `This was so intellectually interesting since I got to read different papers and learn how to build and compare specific ML algorithms. I'm
    so grateful I had Alexander Amini as my mentor, who supported me a ton and further inspired my fascination for how to build ML
    explainability and more unbiased and equitable systems through his work.`,
  },
  {
    company: "MIT Media Lab: CityMatrix Group",
    role: "Undergraduate Researcher",
    dates: "Jan-March 2020",
    description: [
      `Developed computer vision program for a smart home to assist in
    tasks or emergencies for elderly users, using Python and HTML`,
      `Implemented human body pose detection to track standing, sitting, or
    lying positions over time using OpenCV and heuristics`,
    ],
    link: "https://www.media.mit.edu/projects/citymatrix/overview/",
    thanks: `This was my first time contributing to a university research project and I'm super thankful for Dr. Ryan Zhang for the exciting project discussions 
    and for further inspiring my love for social impact tech.`,
  },
];
